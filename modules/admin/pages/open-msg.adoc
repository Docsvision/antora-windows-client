= Открыть страницу карточки в {of-mc}
...
* В {of-mc} xref:6.1@platform:console:section-server.adoc[указан адрес {of-sett-serv}].
* Карточка поддерживает работу с сообщениями.
* Пользователь входит в группу _{dv-web-admin-cns-admins-serv}_ на xref:6.1@platform:admin:config-platform.adoc#config-file[сервере {dv}] и xref:6.2@mgmtconsole:servers:appserver-access.adoc#groups[сервере {of-mc}].
****

Если при нажатии на кнопку image:buttons/admin-console.png[{mc}] возникает ошибка
"Невозможно получить адрес {of-sett-serv}", xref:6.1@platform:console:section-server.adoc[Укажите адрес {of-mc}] в Консоли настройки в формате `\http://example.com:5100`. При указании IP страница сообщений открываться не будет.
